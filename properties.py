...
class GasProperties:
    ...
    def get_effective_transport_properties(self, T, P, Y, k, epsilon, R, r_g_centers, delta_nu, option=config.TURBULENCE_MODEL):
        ...
        v_d = config.RELATIVE_DROPLET_VELOCITY
        ...
        if option == 'CONSTANT' or option == 'FILE':
            ...
            damping_func = 0.0
            D_droplet = 2 * R
            U_rel = config.RELATIVE_DROPLET_VELOCITY
            ...
            if k > 1e-9 and epsilon > 1e-9 and rho > 1e-9:
                l_t_inf = (config.CMU**0.75) * (k**1.5) / epsilon
                l_t_wall = config.VON_KARMAN_CONST * y
            l_t_eff = min(l_t_inf,l_t_wall)

            # u*effの計算
            Re_d = 0.0
            if mu > 1e-9:
                Re_d = rho * U_rel * D_droplet / mu
            # Schiller-Naumann の式による抗力係数 Cd
            Cd = 0.0
            if Re_d > 1e-6 and Re_d < 800:
                Cd = (24.0 / Re_d) * (1.0 + 0.15 * Re_d**0.687)
            elif 800 < Re_d:
                Cd = 0.44
        
            # 相対摩擦速度 U_rel
            u_star_rel = U_rel * np.sqrt(Cd / 2.0)
            ...
            if damping_func < 0.0:
                damping_func = 0.0
            elif damping_func > 1.0:
                damping_func = 1.0

            k_inf = k * (damping_func ** 2)
            k_rel = u_star_rel ** 2
            nu_t_local = ((k_inf+k_rel)**0.5) * l_t_eff
